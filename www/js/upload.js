...
document.getElementById('uploadForm').addEventListener('submit', function (event) {
    ...
    fetch('/www/uploads/', {method: 'POST', body: formData,})
    .then(response => 
    {
    if (response.ok)
...
async function listFiles() {
    try {
        ...
        const response = await fetch('/www/cgi-bin/list-files.py');
        ...
        if (files && files.length > 0) {
            ...
            files.forEach(file => {
                ...
                downloadLink.href = `/www/uploads/${file}`;
...
async function deleteFile(filename) {
    if (confirm(`Are you sure you want to delete ${filename}?`)) {
        try {
            ...
            const response = await fetch(`/www/uploads/${filename}`, {
                method: 'DELETE'
            });
